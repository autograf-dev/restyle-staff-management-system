...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseAdmin
      .from('Transactions')
      .select('"Payment/Date", "Transaction/Tax"')
    ...
    console.log(`Total Tax API - Filter: ${filter}, Records found: ${data?.length || 0}`)

    // Calculate total tax
    const totalTax = data?.reduce((sum, transaction) => {
      const tax = Number(transaction['Transaction/Tax'] || 0)
      return sum + tax
    }, 0) || 0
    ...
    console.error('Error fetching total tax:', e)
    return NextResponse.json({ ok: false, error: 'Failed to fetch total tax' }, { status: 500 })
  }
}
