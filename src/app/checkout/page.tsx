...
function CheckoutContent() {
  ...
  const proceedToCheckout = async () => {
    ...
    try {
      // Simple direct payment processing - simulate payment success and redirect
      setTimeout(() => {
        const successParams = new URLSearchParams({
          appointmentId: appointmentDetails?.id || 'unknown',
          amount: paymentSession.pricing?.totalAmount?.toString() || '0',
          serviceName: appointmentDetails?.serviceName || 'Service',
          customerName: customerInfo?.name || appointmentDetails?.customerName || 'Customer',
          staffName: appointmentDetails?.staffName || 'Staff',
          appointmentDate: appointmentDetails?.startTime || new Date().toISOString(),
          appointmentTime: appointmentDetails?.startTime ? new Date(appointmentDetails.startTime).toLocaleTimeString() : ''
        })
